...
function scoreAnswers(answers) {
  ...
  if (Q2[answers.q2_fehlt]) { const [c, p] = Q2[answers.q2_fehlt]; add(scores, c, p); }
  if (Q3[answers.q3_erster_schritt]) Q3[answers.q3_erster_schritt].forEach(([c, p]) => add(scores, c, p));
  if (Q4[answers.q4_erreicht]) Q4[answers.q4_erreicht].forEach(([c, p]) => add(scores, c, p));
  if (Q5[answers.q5_orientierung]) Q5[answers.q5_orientierung].forEach(([c, p]) => add(scores, c, p));
  if (Q6[answers.q6_aussage]) { const [c, p] = Q6[answers.q6_aussage]; add(scores, c, p); }

  const q8 = Array.isArray(answers.q8_wichtig) ? answers.q8_wichtig : (answers.q8_wichtig ? [answers.q8_wichtig] : []);
  q8.forEach((val) => { if (Q8[val]) Q8[val].forEach(([c, p]) => add(scores, c, p)); });
